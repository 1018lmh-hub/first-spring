...
@Slf4j
@Controller
@RequiredArgsConstructor //생성자(생성자가 아니라 의존성 아닌가) 주입용 매개변수 생성자를 생성해주는 Lombok 에노테이션
public class MemberController {
	...
	*/
	 ...
	 *  2. setter 메서드가 반드시 존재할 것
	 *  3. 전달되는 키값(jsp 네임 속성값)과 객체의 필드명이 동일할 것
	 */
...
//	@Autowired
	...
	*/
	...
	public String login(MemberDto member, HttpSession session, Model model) {
		...
		if(userInfo != null) {
			...
			// forwarding 보다는 => sendRedirect => localhost:8088/kh/
			session.setAttribute("userInfo", userInfo);
			return "redirect:/";
			
			
		} else {
			// requestScope에 실패 메시지를 set 
			// spring에서는 Model에서 request 를 담음
	...
	*/
	...
	// 두번째 방법 : 변환타입을 ModelAndView타입으로 반환
	@PostMapping("/login")
	public ModelAndView login(HttpSession session, ModelAndView mv ,MemberDto member) {
